...
def bcast_list_to_all(list_to_send, myid, source_node = 0, mpi_comm = -1):
	...
	if(myid == source_node):
		...
		if isinstance(list_to_send[0], np.int64):
			tp = 0
		elif isinstance(list_to_send[0], np.float64):
			tp = 1
		elif isinstance(list_to_send[0], types.IntType):
			tp = 2
		elif isinstance(list_to_send[0], types.FloatType):
			tp = 3
		else:
			tp = 4
	else:
	...
	if tp >= 4 :
		ERROR("Only list of the same type numbers can be brodcasted","bcast_list_to_all",1, myid)

	if(myid != source_node):
		if tp == 0 or tp == 1:
			list_to_send = np.array([0]*n)
		elif tp == 2 or tp == 3:
			list_to_send = [0]*n

	if tp == 0:
		list_to_send = list_to_send.astype(np.int32)
		list_to_send = mpi_bcast(list_to_send, n, MPI_INT, source_node, mpi_comm)
		return np.array([int(n) for n in list_to_send], dtype=np.int64)
	elif tp == 1:
		list_to_send = list_to_send.astype(np.float32)
		list_to_send = mpi_bcast(list_to_send, n, MPI_FLOAT, source_node, mpi_comm)
		return np.array([float(n) for n in list_to_send], dtype=np.float64)
	elif tp == 2:
		list_to_send = mpi_bcast(list_to_send, n, MPI_INT, source_node, mpi_comm)
