#!/work/software/sphire/beta_20170602_helical_git/EMAN2/bin/python
#
#  09/09/2016
#  
#  CPU subgroup
...
def params_changes( params, oldparams ):
	...
	from utilities_helix    	import getang3
	from utilities_helix    	import rotate_shift_params
...
def assign_particles_to_groups(minimum_group_size = 10):
	...
	write_text_row(cross_reference_txt[0], os.path.join(Tracker["constants"]["origin_masterdir"],"main000","groupids.txt") )
	write_text_row([[sd[cross_reference_txt[0][i][j]][0] for j in xrange(len(cross_reference_txt[0][i]))]  for i in xrange(len(cross_reference_txt[0]))], os.path.join(Tracker["constants"]["origin_masterdir"],"main000","micids.txt") )
	...
	for iproc in xrange(2):
		write_text_row([cross_reference_txt[0][i] for i in lili[iproc]] , os.path.join(Tracker["constants"]["origin_masterdir"],"main000","groupids_%03d.txt"%iproc) )
		write_text_row([[sd[cross_reference_txt[0][i][j]][0] for j in xrange(len(cross_reference_txt[0][i]))]  for i in lili[iproc]], os.path.join(Tracker["constants"]["origin_masterdir"],"main000","micids_%03d.txt"%iproc) )
	del sd

	write_text_file([len(cross_reference_txt[4][i]) for i in xrange(len(cross_reference_txt[4]))], os.path.join(Tracker["constants"]["origin_masterdir"],"main000","number_of_particles_per_group.txt") )
	...
	write_text_file(g0, os.path.join(Tracker["constants"]["origin_masterdir"],"main000", "particle_groups_0.txt") )
	#write_text_file(q1, os.path.join(Tracker["constants"]["masterdir"],"main000","tchunk_1.txt") )
	write_text_file(g1, os.path.join(Tracker["constants"]["origin_masterdir"],"main000", "particle_groups_1.txt") )
	...
	if( number_of_cones == 1):  return Blockdata["symclass"][1][3] + 0.5, 1
		...
		if( Blockdata["symclass"].sym[0] == "c"):
			...
			for i in xrange(92,0,-1):
				a = Blockdata["symclass"].even_angles(i, theta1=1.0, theta2=t2, method=Tracker['constants']['ref_a'])
			...
			for i in xrange(int(t2+1),0,-1):
				nren = len(Blockdata["symclass"].even_angles(i, theta1=1.0, method=Tracker['constants']['ref_a']))
				if(nren>number_of_cones): return float(i), nren
			while(True):
				i /= 2.0
				nren = len(Blockdata["symclass"].even_angles(i, theta1=1.0, method=Tracker['constants']['ref_a']))
...
def compute_sigma(projdata, params, first_procid, dryrun = False, myid = -1, mpi_comm = -1):
	...
	if  dryrun:
		...
		if( myid == Blockdata["main_node"] ):
			ngroups = len(read_text_file(os.path.join(Tracker["constants"]["origin_masterdir"],"main000", "groupids.txt")))
		else: ngroups = 0
...
def get_shrink_data(nxinit, procid, original_data = None, oldparams = None, \
	...
	from utilities_helix    import get_im, model_gauss_noise, set_params_proj, get_params_proj
...
def get_anger(angle1, angle2):
	...
	from utilities_helix import lacos
...
def get_refangs_and_shifts():
	...
	refang = Blockdata["symclass"].even_angles(Tracker["delta"], method=Tracker['constants']['ref_a'])
	coarse = Blockdata["symclass"].even_angles(2*Tracker["delta"], method=Tracker['constants']['ref_a'])
...
def do3d(procid, data, newparams, refang, rshifts, norm_per_particle, myid, mpi_comm = -1, outliers=None):
	global Tracker, Blockdata

	#  Without filtration
	from reconstruction_helix import recons3d_trl_struct_MPI

	if( mpi_comm < -1 ): mpi_comm = MPI_COMM_WORLD
	"""
	tvol, tweight, trol = recons3d_4nnstruct_MPI(
	...
	tvol, tweight, trol, nr_outlier = recons3d_trl_struct_MPI(
		myid=Blockdata["subgroup_myid"],
		main_node=Blockdata["nodes"][procid],
		prjlist=data,
		paramstructure=newparams,
...
def calculate_2d_params_for_centering(kwargs):
	...
	from utilities_helix 		import wrap_mpi_gatherv, read_text_row, write_text_row, bcast_number_to_all, get_im, combine_params2, model_circle, gather_compacted_EMData_to_root
...
'''
def getangc5(p1,p2):
	from utilities_helix import getfvec
	from math import acos, degrees
...
def Xali3D_direct_ccc(data, refang, shifts, ctfs = None, bckgnoise = None, kb3D = None):
	...
	from utilities_helix 		import wrap_mpi_gatherv
	from math 			import sqrt
		...
		while  cont :
			...
			if(Blockdata["symclass"].sym[0] == "c"):
				if( number_of_cones == 1 ):
					...
					cone_angles = Blockdata["symclass"].even_angles(cone_delta, theta1=1.0, theta2=89.0, method=Tracker['constants']['ref_a'])
					cone_angles += [[(q[0]+90.0)%360., 180.-q[1],0] for q in cone_angles]
					cone_angles = Blockdata["symclass"].reduce_anglesets(cone_angles)
			else:
				cone_angles = Blockdata["symclass"].even_angles(cone_delta, theta1=1.0, method=Tracker['constants']['ref_a'])
...
def do_final_rec3d(partids, partstack, original_data, oldparams, oldparamstructure, projdata, final_iter=-1, comm = -1 ):
	...
	if( Blockdata["subgroup_myid"] > -1 ):
		...
		for procid in xrange(2):
			...
			original_data[procid], oldparams[procid], im_idx[procid][IM_IDX_START], im_idx[procid][IM_IDX_END] = getindexdata(partids[procid], partstack[procid], \
					os.path.join(Tracker["constants"]["origin_masterdir"],"main000", "particle_groups_%01d.txt"%procid), \
			...
			for ipar in xrange(len(oldparams[procid])):
			...
			outlier_file = os.path.join(
				...
				"main{0:03d}".format(Tracker["mainiteration"]-1),
				"outlier-chunk_{0:01d}_{1:03d}.txt".format(
					procid,
					Tracker["mainiteration"]-1
					)
				)
			...
			# Do the calculation
			outliers_node = calculate_prior_values(
				tracker=Tracker,
				blockdata=Blockdata,
				chunk_file=partids[procid],
				outlier_file=outlier_file,
				params_file=partstack[procid],
				im_start=im_idx[procid][IM_IDX_START],
				im_end=im_idx[procid][IM_IDX_END],
				procid=procid
				)
			...
			if Tracker['constants']['apply_prior']:
				if Tracker["state"] == "RESTRICTED" or Tracker["state"] == "FINAL":
					# Do not use outliers during restricted search
					outliers_node = outliers_node
				else:
					# Do use outliers
					outliers_node = [-1 for i in range(len(outliers_node))]
			else:
				# Do use outliers
				outliers_node = [-1 for i in range(len(outliers_node))]

			# Prior calculation finished!
			mpi_barrier(MPI_COMM_WORLD)
			...
			nr_outliers = do3d(
				...
				myid=Blockdata["myid"],
				mpi_comm=comm,
				outliers=outliers_node
				)

			mpi_barrier(MPI_COMM_WORLD)
			# Print outlier information
			nr_outliers = wrap_mpi_gatherv([nr_outliers], Blockdata["main_node"], MPI_COMM_WORLD)
			if(Blockdata["myid"] == Blockdata["main_node"]):
				line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
				print(line,"Did not account", np.sum(nr_outliers), "Particles for the reconstruction")
...
def recons3d_final(masterdir, do_final_iter, memory_per_node):
		...
		if(Blockdata["myid"] == Blockdata["main_node"]):
			...
			try:
				...
				while  os.path.exists(os.path.join(masterdir, "main%03d"%iter_max)) and os.path.exists(os.path.join(masterdir,"main%03d"%iter_max,"Tracker_%03d.json"%iter_max)):
					iter_max +=1
				iter_max -=1
				file_name = os.path.join(masterdir,"main%03d"%iter_max,"Tracker_%03d.json"%iter_max)
				Tracker = load_tracker_from_json(file_name=file_name)
	...
	if(Blockdata["myid"] == Blockdata["main_node"]): # check json file and load tracker
		try:
			file_name = os.path.join(final_dir,"Tracker_%03d.json"%do_final_iter)
			Tracker = load_tracker_from_json(file_name=file_name)
		except:
			carryon = 0
	else:
...
def do_ctrefromsort3d_get_subset_data(masterdir, option_old_refinement_dir, option_selected_cluster, option_selected_iter, shell_line_command):
	...
	if Blockdata["myid"] == Blockdata["main_node"]:
		file_name = os.path.join(old_refinement_iter_dir, "Tracker_%03d.json"%selected_iter)
		Tracker = load_tracker_from_json(file_name=file_name)
	else: Tracker = 0
	...
	if Blockdata["myid"] == Blockdata["main_node"]:# some numbers and path are required to be modified
		...
		file_name = os.path.join(iter_dir, "Tracker_%03d.json"%selected_iter)
		dump_tracker_to_json(tracker=Tracker, file_name=file_name)
...
def ctrefromsorting_rec3d_faked_iter(masterdir, selected_iter=-1, comm = -1):
	...
	if Blockdata["myid"] == Blockdata["main_node"]:
		file_name = os.path.join(Tracker["directory"], "Tracker_%03d.json"%selected_iter)
		Tracker = load_tracker_from_json(file_name=file_name)
	else: Tracker = 0
	...
	for procid in xrange(2):
		...
		mpi_barrier(MPI_COMM_WORLD)
		#####
		original_data[procid], oldparams[procid], im_start, im_end = getindexdata(partids[procid], partstack[procid], \
				os.path.join(Tracker["constants"]["origin_masterdir"],"main000", "particle_groups_%01d.txt"%procid), \
				original_data[procid], small_memory = Tracker["constants"]["small_memory"], \
...
def update_tracker(shell_line_command):
	...
	parser_no_default.add_option("--apply_prior",            action="store_true")
	...
	if  options_no_default_value.apply_prior != False:
		Tracker["constants"]["apply_prior"] = options_no_default_value.apply_prior
...
def dump_tracker_to_json(tracker, file_name):
	"""Dump the tracker information to json format"""
...
def load_tracker_from_json(file_name):
	"""Load the tracker information from json format"""

	# Load original tracker
	with open(file_name, 'r') as read:
		tracker = convert_json_fromunicode(json.load(read))

	# Load numpy tracker
	numpy_tracker_file = get_numpy_tracker(tracker, file_name)
	with open(numpy_tracker_file, 'r') as read:
		numpy_tracker = convert_json_fromunicode(json.load(read))

	# Fill normal tracker with numpy information
	tracker = search_dict(tracker, numpy_tracker)

	return tracker


def search_dict(dict_1, dict_2):
	for key in dict_1:
		if isinstance(dict_1[key], dict):
			dict_1[key] = search_dict(dict_1[key], dict_2)
			continue
		else:
			pass

		if dict_1[key] == 'NUMPY_TRACKER_STACK':
			dict_1[key] = convert_list_to_numpy(
				dict_2[key],
				ms_helix_sphire.get_stack_dtype()
				)
			continue
		else:
			pass
...
def get_numpy_tracker(tracker, file_name):
	"""Create numpy tracker file name"""
	out_tracker = file_name.split('/')[-1]
	out_dir = file_name[:-len(out_tracker)-1]
	numpy_tracker_file = '{0}/{1}{2}'.format(
		out_dir,
		tracker['constants']['numpy_tracker_prefix'],
		out_tracker
		)
	return numpy_tracker_file


def convert_list_to_numpy(data, dtype):
	temp = np.empty(len(data), dtype=dtype)
	dtype_names = temp.dtype.names
	for idx, entry in enumerate(data):
		for index, element in enumerate(entry):
			temp[idx][dtype_names[index]] = element
	return temp
...
def calculate_prior_values(tracker, blockdata, outlier_file, chunk_file, params_file, im_start, im_end, procid):
	...
	if blockdata["myid"] == blockdata["nodes"][0]: 
		...
		print('---> Chunk {0}: Discarded {1}|{2:.1f}%; Kept {3}|{4:.1f}%; Nr. particles {5}'.format(
			procid,
			nr_outliers,
...
def debug(line, debug, **kwargs):
	global Blockdata, Tracker
	if Blockdata['myid'] == Blockdata['main_node'] and debug:
		for key in kwargs:
			print(line, key, ':', kwargs[key])

def main():

	from utilities_helix import write_text_row, drop_image, model_gauss_noise, get_im, set_params_proj, wrap_mpi_bcast, model_circle
	...
	if not update_options: #<<<-------Fresh run
		...
		Constants["origin_masterdir"]           	= masterdir
		Constants["best"]              			= 0
	...
	# ------------------------------------------------------------------------------------
		...
		initdir = os.path.join(Tracker["constants"]["origin_masterdir"],"main000")
		...
		if not options.ctrefromsort3d:
			...
			keepchecking 		= 1
			if(Blockdata["myid"] == Blockdata["main_node"]):
				file_name = os.path.join(initdir,"Tracker_000.json")
				Tracker = load_tracker_from_json(file_name=file_name)
			else:
				Tracker = None
			Tracker 	= wrap_mpi_bcast(Tracker, Blockdata["main_node"])
	...
	keepgoing = 1
	if( Blockdata["myid"] == Blockdata["main_node"] ):
		file_name = os.path.join(Tracker["constants"]["masterdir"],"main%03d"%Tracker["mainiteration"],"Tracker_%03d.json"%Tracker["mainiteration"])
		dump_tracker_to_json(tracker=Tracker, file_name=file_name)

	continue_mode = False
	continue_mode_first = False
	while(keepgoing):
		...
		if doit :
			if(Blockdata["myid"] == Blockdata["main_node"]):
				file_name = os.path.join(Tracker["previousoutputdir"],"Tracker_%03d.json"%(Tracker["mainiteration"]-1))
				Tracker = load_tracker_from_json(file_name=file_name)
				#  It has to be repeated here as Tracker is from previous iteration, I see no other way.
				Tracker["previousoutputdir"]	= os.path.join(Tracker["constants"]["masterdir"],"main%03d"%(int(Tracker["mainiteration"])))
				Tracker["mainiteration"]	= mainiteration
				Tracker["directory"]		= os.path.join(Tracker["constants"]["masterdir"],"main%03d"%Tracker["mainiteration"])
				if continue_mode_first:
					...
					line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
					print("\n\n\n\n")
					print(line,"Continue run detected!")
					Tracker["constants"]["masterdir"] = os.path.join(temp_tracker["constants"]["origin_masterdir"], temp_tracker["constants"]["continue_name"])
					if os.path.exists(Tracker["constants"]["masterdir"]):
						line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
						print(line,"Continue directory {0} already exists! Finding new name!".format(Tracker["constants"]["masterdir"]))
						for i in range(9999):
							Tracker["constants"]["masterdir"] = os.path.join(temp_tracker["constants"]["origin_masterdir"], '{0}_{1}'.format(temp_tracker["constants"]["continue_name"], i))
							if not os.path.exists(Tracker["constants"]["masterdir"]):
								break
					os.mkdir(Tracker["constants"]["masterdir"])

					masterdir = Tracker["constants"]["masterdir"]
					Tracker["directory"] = os.path.join(Tracker["constants"]["masterdir"], "main{0:03d}".format(temp_tracker["mainiteration"]))

					shutil.copytree(Tracker["previousoutputdir"], os.path.join(Tracker["constants"]["masterdir"], "main%03d"%(int(Tracker["mainiteration"])-1)))
					Tracker["previousoutputdir"] = os.path.join(Tracker["constants"]["masterdir"], "main%03d"%(int(Tracker["mainiteration"])-1))

					shutil.copytree(os.path.join(Tracker["constants"]["origin_masterdir"], "main000"), os.path.join(Tracker["constants"]["masterdir"], "main000"))
					...
					print_dict(Tracker["constants"],"Values for the continue run")
					print("\n\n\n\n")
					continue_mode_first = False
			else:
				Tracker = None
			...
			if keepgoing == 1: # not converged
				...
				for procid in xrange(2):
					...
					#  store params
					if(Blockdata["myid"] == Blockdata["main_node"]):
						chunk_file = os.path.join(Tracker["previousoutputdir"], "chunk_%01d_%03d.txt"%(procid,Tracker["mainiteration"]-1))
						...
						write_text_row(params, params_file)
					del params

					projdata[procid] = []
					if Tracker["constants"]["small_memory"]:
						original_data[procid], oldparams[procid], im_idx[procid][IM_IDX_START], im_idx[procid][IM_IDX_END] = getindexdata(
							partids[procid],
							partstack[procid],
							os.path.join(Tracker["constants"]["origin_masterdir"],"main000",
					...
					chunk_file = os.path.join(
						Tracker["previousoutputdir"],
						"chunk_{0:01d}_{1:03d}.txt".format(
							procid,
							Tracker["mainiteration"]-1
							)
						)
					params_file = os.path.join(
						Tracker["directory"],
						"params-chunk_{0:01d}_{1:03d}.txt".format(
							procid,
							Tracker["mainiteration"]
							)
						)
					...
					# Print outlier information finished
					mpi_barrier(MPI_COMM_WORLD)
				...
				#		if( Blockdata["color"] == Blockdata["node_volume"][1] ):
				#			if( Blockdata["myid_on_node"] == 0 ):
				#				outliers_0 = ms_helix_fundamental.calculate_priors(tracker=Tracker, params_file=partstack[0], index_file=partids[0], plot=True, node=0)
				#				in_0 = len(outliers_0[outliers_0 == 0])
				#				all_0 = len(outliers_0)
				#				out_0 = all_0 - in_0
				#				print('---> Chunk 0: Discarded {0}|{1}%; Kept {2}|{3}%; Nr. particles {4}'.format(
				#					out_0, 100*out_0/all_0, in_0, 100*in_0/all_0, all_0
				#					))
				#	if( iorder == lorder[1] ):
				#		if( Blockdata["color"] == Blockdata["node_volume"][0] ):
				#			if( Blockdata["myid_on_node"] == 0 ):
				#				outliers_1 = ms_helix_fundamental.calculate_priors(tracker=Tracker, params_file=partstack[1], index_file=partids[1], plot=True, node=1)
				#				in_1 = len(outliers_1[outliers_1 == 0])
				#				all_1 = len(outliers_1)
				#				out_1 = all_1 - in_1
				#				print('---> Chunk 1: Discarded {0}|{1}%; Kept {2}|{3}%; Nr. particles {4}'.format(
				#					out_1, 100*out_1/all_1, in_1, 100*in_1/all_1, all_1
				#					))
				#for iorder in xrange(2):
				#	if( iorder == lorder[0] ):
				#		if( Blockdata["color"] == Blockdata["node_volume"][1] ):
				#			if( Blockdata["myid_on_node"] == 0 ):
				#				user_func_prior = user_functions.factory[Tracker["constants"]["user_func_prior"]]
				#				ref_data_prior = [Tracker, partstack[0], partids[0]]
				#				user_func_prior(ref_data_prior)
				#	if( iorder == lorder[1] ):
				#		if( Blockdata["color"] == Blockdata["node_volume"][0] ):
				#			if( Blockdata["myid_on_node"] == 0 ):
				#				user_func_prior = user_functions.factory[Tracker["constants"]["user_func_prior"]]
				#				ref_data_prior = [Tracker, partstack[1], partids[1]]
				#				user_func_prior(ref_data_prior)
				# COMMENT OLD CODE
				mpi_barrier(MPI_COMM_WORLD)
				...
				if(Tracker["mainiteration"] == 1 ):
					...
					if(Blockdata["myid"] == Blockdata["main_node"]):
						...
						particle_groups = read_text_file(os.path.join(Tracker["constants"]["origin_masterdir"],"main000", "particle_groups_0.txt") ) + read_text_file(os.path.join(Tracker["constants"]["origin_masterdir"],"main000", "particle_groups_1.txt") )
				...
				if(Blockdata["myid"] == Blockdata["main_node"]):
					file_name = os.path.join(masterdir,"main%03d"%Tracker["mainiteration"],"Tracker_%03d.json"%Tracker["mainiteration"]) # AI already correctly set Tracker["mainiteration"]
					Tracker_final_iter = load_tracker_from_json(file_name=file_name)
					line = strftime("%Y-%m-%d_%H:%M:%S", localtime()) + " =>"
					if Tracker_final_iter["bestres"] <= Tracker["bestres"]: # need an update even it is equal
						...
						file_name = os.path.join(masterdir,"main%03d"%Tracker["mainiteration"],"Tracker_%03d.json"%Tracker["mainiteration"])
						dump_tracker_to_json(tracker=Tracker_final_iter, file_name=file_name)
