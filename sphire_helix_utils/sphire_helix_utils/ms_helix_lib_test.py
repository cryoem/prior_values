...
class Test_import_data_sphire(unittest.TestCase):
    ...
    def test_tracker_is_filename_with_filamentID_segmentID_ISAC_class_id(self):
        prior_tracker = self.create_prior_tracker(tracker="bdb:stack_withsegmentID_filamentID_ISAC_classID", has_ISAC_class_id=True)
        ...
        aspected_output = ('filt_Factin_ADP_cLys_0009_falcon2.hdf', 'filt_Factin_ADP_cLys_0009_falcon2.hdf0009', 7, 38.40719, 82.93683, 298.12543, -7.48667, 15.25721, 0.5188, 0.73356, 90890.99319, 683, 683)
        self.assertTrue(aspected_output, prior_tracker['array'][-1])
        aspected_output = ('ptcl_source_image', 'data_n', 'ISAC_class_id', 'filament_id', 'segment_id', 'phi', 'theta', 'psi', 'shift_x', 'shift_y', 'err1', 'err2', 'norm', 'source_n', 'stack_idx')
        self.assertTrue(aspected_output == prior_tracker['array'].dtype.names)
        clean_up([self.params_file, self.index_file])
    ...
    def test_tracker_is_dictionary_and_contains_array_with_filamentID_segmentID_ISAC_class_id(self):
        prior_tracker = self.create_prior_tracker(tracker = "bdb:stack_withsegmentID_filamentID_ISAC_classID", has_ISAC_class_id=True)
        tracker = {'constants': {'stack_prior': prior_tracker['array'][['ptcl_source_image', 'filament_id', 'segment_id', 'data_n']]}}
        ...
        aspected_output = ('filt_Factin_ADP_cLys_0009_falcon2.hdf', 'filt_Factin_ADP_cLys_0009_falcon2.hdf0009', 7,  38.40719,  82.93683,  298.12543, -7.48667,  15.25721,  0.5188,  0.73356,  90890.99319, 683, 683)
        self.assertTrue(aspected_output, prior_tracker['array'][-1])
        aspected_output = ('ptcl_source_image', 'filament_id', 'segment_id', 'data_n', 'phi', 'theta', 'psi', 'shift_x', 'shift_y', 'err1', 'err2', 'norm', 'source_n', 'stack_idx')
        self.assertTrue(aspected_output == prior_tracker['array'].dtype.names)
        clean_up([self.params_file, self.index_file])
    ...
    def test_tracker_is_dictionary_and_contains_filenamewith_filamentID_segmentID_ISAC_class_id(self):
        tracker = {'constants': {'stack': "bdb:stack_withsegmentID_filamentID_ISAC_classID"}}
        prior_tracker = self.create_prior_tracker(tracker=tracker, has_ISAC_class_id=True)
